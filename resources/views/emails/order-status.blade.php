...
<body style="font-family: Arial, sans-serif; background:#f5f6fa; padding:24px; margin:0;">
    <div style="max-width:560px; margin:0 auto; background:#ffffff; border-radius:12px; overflow:hidden; border:1px solid #eaeaea;">
        ...
        @php
            ...
            $accent       = $isCompleted ? '#1e8e3e' : '#c62828';
            $accentBg     = $isCompleted ? '#f0fbf3' : '#fdf1f0';
            $accentBorder = $isCompleted ? '#cdeed7' : '#f5cfcb';
            $verb         = strtolower($type) === 'sell' ? 'Sell' : 'Buy';
            $icon         = $isCompleted ? '&#10003;' : '&#10005;';
        @endphp
            ...
            <tr>
                <td style="padding:16px 32px;">
                    <table cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
                        <tr>
                            <td style="width:26px; height:26px; border-radius:50%; background:{{ $accent }}; color:#fff; font-size:13px; font-weight:700; text-align:center; vertical-align:middle;">{!! $icon !!}</td>
                            <td style="padding-left:10px; color:{{ $accent }}; font-weight:700; font-size:14px; letter-spacing:.3px; text-transform:uppercase;">Order {{ $status }}</td>
                        </tr>
                    </table>
                </td>
            </tr>
        ...
        <div style="padding:32px;">
            ...
            <h2 style="margin:0 0 16px; color:#222;">
                {{ $isCompleted ? "Your {$verb} Order Is Complete" : "Your {$verb} Order Was Cancelled" }}
            </h2>
            <p style="color:#444; font-size:15px; margin:0 0 20px;">
                @if ($isCompleted)
                    Great news — your order to {{ strtolower($verb) }} shares of <strong>{{ $stockName }}</strong> has been successfully completed.
                @else
                    Your order to {{ strtolower($verb) }} shares of <strong>{{ $stockName }}</strong> has been cancelled. If you have questions, our team is happy to help.
                @endif
            </p>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse; margin-bottom:20px;">
                ...
                <tr>
                    ...
                    <td style="padding:10px 0; border-bottom:1px solid #eee; color:#222; font-size:13px; text-align:right; font-weight:700; text-transform:uppercase;">{{ $verb }}</td>
                </tr>
                <tr>
            ...
            <p style="color:#888; font-size:13px; margin:0;">
                @if ($isCompleted)
                    Thank you for trading with UnlistedGain. Reach out anytime if you need help with settlement or your next order.
                @else
                    You can place a new request anytime from your dashboard, or contact our team for assistance.
                @endif
            </p>
        </div>
